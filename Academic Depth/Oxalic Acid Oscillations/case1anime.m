...
%% Video Setup
filename = 'Case 1 theta time profile';  % Output video filename
v = VideoWriter(filename);           % Create video writer object

v.FrameRate = 4;                     % Frames per second (adjust as needed)
...
for e = 0:0.01:0.4
    ...
    % Calculate concentration profile C(x)
    %     x = linspace(0, delta, 100); % Spatial points
    %     C = (k1*(1-theta(end))/D) * (C_bulk / (1 + (k1*delta*(1-theta(end)))/D)) * x + (C_bulk / (1 + (k1*delta*(1-theta(end)))/D));
    %
    ...
    title(['Surface Coverage vs. Time, eta = ' num2str(eta)]);
    grid on;
...
%% Conc profile
filename = 'Case 1 Concentration profile';  % Output video filename
v = VideoWriter(filename);           % Create video writer object
v.FrameRate = 4;                     % Frames per second (adjust as needed)
open(v);                             % Open the video writer
figure(2)
...
for i = 2:(((nt-1)/1000)-1)
    ...
    % Update coverage using forward Euler
    theta(i+1) = theta(i) + dt*dtheta_dt*1000;
    
    % Ensure theta stays within bounds [0, 1]
    theta(i+1) = max(0, min(1, theta(i+1)));
    % Calculate concentration profile C(x)
    x = linspace(0, delta, 100); % Spatial points
    C = (k1*(1-theta(i))/D) * (C_bulk / (1 + (k1*delta*(1-theta(i)))/D)) * x + (C_bulk / (1 + (k1*delta*(1-theta(i)))/D));
    %disp(theta);
    subplot(1,2,1)
    plot(time, theta);
    xlabel('Time (s)');
    ...
    ylim([0.99,1])
    xlim([0,1e-4])
    title(['Concentration Profile at time t = ' num2str(i/100)]);
    grid on;
    frame = getframe(gcf);
    writeVideo(v, frame);
    pause(0.1)
